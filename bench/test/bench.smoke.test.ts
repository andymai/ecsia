...
describe('bench builders advance state', () => {
  test('each iterate builder integrates Position from Velocity', () => {
    for (const make of [makeEcsiaIter, makeEcsiaCursorIter, makeEcsiaPinnedIter, makeMiniplexIter, makeBitEcsIter]) {
      const c = make(1000)
  ...
  // The cursor and pinned rows are the showcased fast paths; each must integrate the SAME data the
  // accessor path does ABOVE the 1024 column-growth boundary (the size the macro-bench actually
  // runs). A fast-but-wrong loop would pass below 1024 and silently report a misleading number above
  // it (the prior bug), so this pins both against the accessor at n>1024 across multiple steps.
  test('ecsia fast-path results equal the ecsia accessor result above the 1024 growth boundary', () => {
    for (const makeFast of [makeEcsiaCursorIter, makeEcsiaPinnedIter]) {
      for (const n of [1025, 5000]) {
        const accessor = makeEcsiaIter(n)
        const fast = makeFast(n)
        for (let i = 0; i < 10; i++) {
          accessor.step()
          fast.step()
          expect(fast.sampleX()).toBeCloseTo(accessor.sampleX(), 9)
        }
        // Both start at x=0, dx=1, dt=1/60: after 10 steps x must be 10/60, NOT a halved/aliased value.
        expect(fast.sampleX()).toBeCloseTo(10 / 60, 6)
      }
    }
  })
...
describe('bench runner emits a report', () => {
  ...
  test(
    ...
    async () => {
      ...
      expect(report.iterate.map((r) => r.name)).toEqual(['ecsia', 'ecsia-cursor', 'ecsia-pinned', 'miniplex', 'bitECS'])
