...
// Proves the fast-check runner is wired into CI and shrinking works.
test('fast-check property runner is wired', () => {
  fc.assert(fc.property(fc.integer(), (n) => Number.isInteger(n)))
})

// A real invariant: any valid maxEntities within the index space resolves and round-trips.
