...
>(
  ...
  if (options?.brand === undefined && options?.name === undefined) {
    throw new Error("defineComponent: a 'name' option is required — it's the key you read the component by (entity.<name>) and keeps separate components from colliding")
  }
