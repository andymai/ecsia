// pairKey64: the logical 64-bit key for the (relationId, targetIndex) mint map.
// Keyed by the target's INDEX (low handle bits), not the full handle, so a pair id is stable across
// the target slot's generation bumps. The bigint is a MAP KEY ONLY — the stored artifact is
...
// targetIndex the world's handleIndex produces. The high field (relationId / subjectIndex) is
...
/** (relationId << indexBits) | targetIndex — the logical mint-map key. */
...
/** (subjectIndex << indexBits) | targetIndex — the overflow-table row key. */
