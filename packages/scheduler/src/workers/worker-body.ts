// The transport-neutral worker thread body. Everything the dispatch loop does — bootstrapping the
// zero-copy world view from the SAB manifest, the Atomics.wait wake loop, kernel dispatch, command
// buffering, topic consume, column re-backing — is host-agnostic: SAB + Atomics behave identically
// under node:worker_threads and browser Web Workers (blocking waits are legal on any worker thread).
// The entries (worker-entry.ts for Node, browser-entry.ts for the web) differ ONLY in how the
// bootstrap arrives (workerData vs first postMessage), how kernels are obtained (dynamic
// kernelModule import vs statically bundled builder), and the message port surface — abstracted
// here as WorkerHostPort. ONE body, two hosts: the serial-equivalence guarantee stays single-sourced.
...
/** The kernels a worker runs, keyed by system name, plus the defs whose ids align to the manifest. */
export interface WorkerKernelsBundle {
  readonly kernels: Map<string, WorkerSystemKernel>
  readonly components: Map<string, ComponentDef<Schema>>
  /** Worker-side defineTopic defs, aligned to the main thread's topic ids by name. */
  readonly topics?: Map<string, TopicDef<Schema>>
}

/** The kernel-module contract (`kernelModule` option): a module exporting buildWorkerKernels(). */
export interface KernelModule {
  buildWorkerKernels(): WorkerKernelsBundle
}

/**
 * The host-side message surface a worker body needs: outbound diagnostics/ready, and the inbound
 * handler. Single consumer, installed at most once (the body installs exactly one) — the Node
 * adapter appends a listener, while the browser adapter reassigns self.onmessage, deliberately
 * replacing the bootstrap handler that started the body. Node adapts parentPort; the browser
 * adapts self.
 */
...
const WAKE = 0
/** Work descriptor SAB layout: [0]=systemId [1]=count [2..3]=dt(f64) [4..]=entity indices (byte 16). */
const WORK_INDICES_BYTE_OFFSET = 16

/**
 * Run the worker dispatch loop until the shutdown sentinel (wake word < 0). The loop:
 *
 * 1. Atomics.wait on the wake word for the next dispatch generation (blocking wait, OFF the
 *    main thread, so a blocking main thread may itself wait on the wave fence without deadlock).
 ...
 *    (the last decrementer notifies the waiter).
 *
 * The worker NEVER mutates shared structure mid-wave and NEVER reads the bitmask.
 */
export async function runWorkerBody(boot: WorkerBootstrap, bundle: WorkerKernelsBundle, port: WorkerHostPort): Promise<void> {
  const { kernels, components: defByName, topics: topicByName } = bundle
  // The SystemId→name mapping is the POOL's registration order (the bootstrap), NOT the kernel
  // bundle's own order — so the worker runs the system the pool dispatched, by name.
  ...
  // bundle's defineComponent calls leave id UNREGISTERED; the manifest carries the authoritative ids.
  ...
  const cb: CommandBuffer = {
    ...
    // shared backing. On overflow `ensureWords` caps and sets `overflowed`.
    fixed: true,
    overflowed: false,
  }
  ...
  const encoder = makeEncoder({
    ...
    warn(message) {
      port.postMessage({ kind: 'diagnostic', message })
    },
  })

  // OP_CREATE on the worker takes its handle from the SAB reservation cursor (Atomics.sub take path).
  // On exhaustion takeReserved returns NO_ENTITY (0xffffffff): do NOTHING and return NO_ENTITY — NOT
  // route a NO_ENTITY handle through baseCreate (which would append a spurious OP_CREATE 0xffffffff
  // that the apply path would try to spawn → record-table corruption).
  ...
  ;(encoder as { create: typeof baseCreate }).create = (): ReturnType<typeof baseCreate> => {
    ...
    if (!ensureWords(cb, 2)) {
      if (!cb.overflowWarned) {
        ...
        port.postMessage({ kind: 'diagnostic', message: 'command-buffer: fixed (SAB) buffer full; record dropped (raise commandWords)' })
      }
      return NO_ENTITY
    }
    const h = takeReserved(reservation)
    if ((h as unknown as number) >>> 0 === NO_ENTITY_BITS) {
      port.postMessage({ kind: 'diagnostic', message: 'command-buffer: reservation exhausted; raise maxSpawnsPerWave (spawn capped)' })
      return h
    }
  ...
  const workF64 = new Float64Array(boot.workSab, 8, 1)
  ...
  port.onMessage((msg: unknown) => {
    if ((msg as { kind?: string })?.kind === 'columns-added') {
  ...
  port.postMessage({ kind: 'ready', workerIndex: boot.workerIndex })
  ...
  while (true) {
    ...
    if (publishedGen !== appliedNoticeGen) {
      try {
        ...
        port.postMessage({ kind: 'error', message: String(err) })
      }
    ...
    if (cb.overflowed) {
      ...
      port.postMessage({ kind: 'diagnostic', message: `command-buffer overflow on worker ${boot.workerIndex}; raise commandWords (records capped this wave)` })
    }
