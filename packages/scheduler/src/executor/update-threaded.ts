// The THREADED frame loop (scheduler.md §6.2 + §7, PHASE-2): the same wave/round walk as the
...
//
// This is the missing integration the M7 headline requires: a threaded world REPRODUCES the
// single-thread observable result through the SAME frame loop (scheduler.md §2.2/§6.5), not only
...
// dispatch, while world.phase is still 'serial' (so their direct-apply fast path is legal, §6.4).
//
// Because batches in a round are conflict-free by construction (WAVE-CONFLICT §5.2) and the command
// merge order is fixed (§5.5), the threaded result equals the single-thread result for the same
// (state, plan, dt).
...
async function runWaveThreaded(env: ExecutorEnv, pool: RoundDispatcher, wave: ScheduleWave, dt: number): Promise<void> {
  for (const round of wave.rounds) {
    // Main-thread batches first (phase stays 'serial', direct-apply legal — PHASE-2 §6.4).
 ...
 * the only difference is that each round's worker batches run on workers (scheduler.md §6.2).
 */
